...
void testCS(long thread_id)
{
    ...
    for (i=0;i<k;i++){
        ...
        // Bounded CAS implemented from os-book
        waiting[thread_id-1] = true;
        key = 1;
        while (waiting[thread_id-1]==true
         && key == 1){
            int expected = 0;
            if (atomic_compare_exchange_strong(&lock_stream, &expected, 1)) key=0;
        }
        ...
        if (j == thread_id-1) lock_stream = 0;
...
{   
    ...
    AvgTime = (long *)malloc((n)*sizeof(long));
    WorstTime = (long *)malloc((n)*sizeof(long));

    // Initializing AvgTime and WorstTime array to 0.
    for (int i=0;i<n;i++) AvgTime[i] = WorstTime[i] = 0;

    
    waiting = (bool *)malloc((n)*sizeof(int));
    ...
    free(waiting);          // free the memory allocated to waiting arr
