...
outputFile="oNumTrans.txt"
echo "totalDelay AbortCount" > $outputFile

for i in {1..5}
do
    echo "16 1000 $((i*1000)) 100 20 0.7" > inp-params.txt
    # print input parameters using cat
    cat inp-params.txt
    echo "numTrans = $((i*1000)) BTO"
    echo "numTrans = $((i*1000)) BTO" >> $outputFile
    for j in {1..3}
    do
        # print system time
    ...
    echo "numTrans = $((i*1000)) MVTO"
    echo "numTrans = $((i*1000)) MVTO" >> $outputFile
    for j in {1..3}
    do
        # print system time
        echo "System time: $(date)"
    ...
    echo "numTrans = $((i*1000)) MVTO-gc"
    echo "numTrans = $((i*1000)) MVTO-gc" >> $outputFile
    for j in {1..3}
    do
        # print system time
        echo "System time: $(date)"
    ...
    echo "numTrans = $((i*1000)) k-MVTO"
    echo "numTrans = $((i*1000)) k-MVTO" >> $outputFile
    for j in {1..3}
    do
        # print system time
        echo "System time: $(date)"
