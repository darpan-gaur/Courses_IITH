...
// global variables
int n, m, numTrans, constVal, k_size=5;
...
class MVTOscheduler {
    public:
        ...
        int write(int i, int x, int l) {
            ...
            // if (dataItems[x]->W_TS.size()>k_size) {
            //     // remove smallest element
            //     auto it = dataItems[x]->W_TS.begin();
            //     dataItems[x]->W_TS.erase(it);
            // }
...
// open output file
FILE *logFile = fopen("kMVTOlog.txt", "w");
...
int main() {
    ...
    fscanf(inFile, "%d %d %d %d %lf %lf %d", &n, &m, &numTrans, &constVal, &lambda, &readRatio, &k_size);
