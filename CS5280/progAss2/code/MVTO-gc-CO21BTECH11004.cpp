...
class MVTOscheduler {
    public:
        ...
        void garbage_collect() {
            ...
            int minActive = INT_MAX;
            for (auto x : tList) {
                if (x.second->tStatus == 0) {
                    minActive = min(minActive, x.first);
                }
            }
            // get max write timestamp less than minActive
            int maxWTS = INT_MIN;
            ...
            for (auto x : dataItems) {
                // obtain lock
                x->dataItemLock.lock();
                // delete all W_TS less than maxWTS
                for (auto k : x->W_TS) {
                    if (k < maxWTS && tList[k]->tStatus != 0){
                        x->W_TS.erase(k);
                    }
                }

                // delete all R_TS less than maxWTS
                for (auto [j, k] : x->R_TS) {
                    if (k < maxWTS && tList[j]->tStatus != 0) {
                        x->R_TS.erase({j, k});
                    }
                }
                x->dataItemLock.unlock();
            }
            schLock.unlock();
        }
...
// open output file
FILE *logFile = fopen("MVTO-gcLog.txt", "w");
