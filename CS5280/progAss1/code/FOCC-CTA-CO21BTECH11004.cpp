    ...
    public:
        ...
        int tryCommit(int i) {
            ...
            */
            ...
            set<int> wIntersectRn; // read-write intersection
            // lock data item in increasing order of id
            for (auto x : tList[i]->writeSet) {
                dataItems[x]->dataItemLock.lock();

                for (auto t : dataItems[x]->readList) {
                    if (t->tStatus) continue; // if not committed
                    wIntersectRn.insert(t->tID);
                }
            }
            // remove current transaction if present
            wIntersectRn.erase(i);

            if (wIntersectRn.size()==0) {
                // commit
                transaction* t = tList[i];
...
foccCTAScheduler* scheduler;

// open output file
FILE *logFile = fopen("foccCTALog.txt", "w");
...
int main() {
    ...
    scheduler = new foccCTAScheduler();
