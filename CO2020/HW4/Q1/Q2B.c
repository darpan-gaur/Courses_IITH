...
void calc_sources(int nx, int ny, double *x, double *y, double *dx, double *dy, double **T, double **b)
{
  ...
  for(i=0; i<nx; i++)
   for(j=0; j<ny; j++)
   {
     b[i][j] = -(1+0.8*(x[i]*(1-x[i]))*cos(PI*y[j]))*(cos(PI*y[j]*(2+PI*PI*(x[i]*(1-x[i]))))) + 0.8*(((1-2*x[i])*cos(PI*y[j]))*((1-2*x[i])*cos(PI*y[j])) + (PI*(x[i]*(1-x[i]))*sin(PI*y[j]))*(PI*(x[i]*(1-x[i]))*sin(PI*y[j])));                      // set source function here
