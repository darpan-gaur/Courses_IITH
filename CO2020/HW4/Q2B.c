...
void calc_sources(int nx, int ny, double *x, double *y, double *dx, double *dy, double **T, double **b)
{
    ...
    for (i = 0; i < nx; i++){
        ...
        {
            double k = A + B * T[i][j];
            b[i][j] = B * (pow(1.0 - 2.0 * x[i], 2.0) * pow(cos(PI * y[j]), 2.0) + pow(PI, 2.0) * pow(x[i], 2.0) * pow(1.0 - x[i], 2.0) * pow(sin(PI * y[j]), 2.0)) + k * (-2.0 * cos(PI * y[j]) - pow(PI, 2.0) * x[i] * (1.0 - x[i]) * cos(PI * y[j])); // set source function here
...
void get_coeffs(int nx, int ny, double *x, double *y, double *dx, double *dy, double **aP, double **aE, double **aW, double **aN, double **aS, double **b, double **T, double **kdiff, double **bcleft, double **bcrght, double **bctop, double **bcbot)
{
  ...
  for(i=1; i<nx-1; i++)
   for(j=1; j<ny-1; j++)
   {
      Txdx = T[i+1][j] - T[i-1][j];
      Tydy = T[i][j+1] - T[i][j-1];
      aP[i][j] = 2.0 * (1.0 + hxhy_sq) * kdiff[i][j];
      aE[i][j] = -kdiff[i][j] - 0.2*Txdx;       aW[i][j] = -kdiff[i][j] + 0.2*Txdx;
      aN[i][j] = (-kdiff[i][j] - 0.2*Tydy) * hxhy_sq;   aS[i][j] = (-kdiff[i][j] + 0.2*Tydy) * hxhy_sq;
   }
  ...
  i = 0;
   ...
   {  
      ...
      Txdx = T[i+1][j] - T[i][j];
      if (j==0) Tydy = T[i][j+1] - T[i][j];
      else if (j==ny-1) Tydy = T[i][j] - T[i][j-1];
      else Tydy = T[i][j+1] - T[i][j-1];

      aP[i][j] = 2.0 * (1.0 + hxhy_sq) * kdiff[i][j] * pj + 2.0 * dx[0] * qj * (-kdiff[i][j] + 0.2 * Txdx);
      aE[i][j] = -2.0 * kdiff[i][j] * pj;   aW[i][j] = 0.0;
      aN[i][j] = (-kdiff[i][j] - 0.2*Tydy) * hxhy_sq * pj;   aS[i][j] = (-kdiff[i][j] + 0.2*Tydy) * hxhy_sq * pj;
      b[i][j] =  b[i][j] * pj  + 2.0 * rj * dx[0] * (-kdiff[i][j] + 0.2 * Txdx);
   }
  ...
  i = nx-1;
   ...
   {  
     ...
     pj = bcrght[j][0]; qj = bcrght[j][1]; rj = bcrght[j][2];
      ...
      aP[i][j] = 2.0 * (1.0 + hxhy_sq) * kdiff[i][j] * pj - 2.0 * dx[0] * qj * (-kdiff[i][j] - 0.2 * Txdx);
      aE[i][j] = 0.0;   aW[i][j] = -2.0 * kdiff[i][j] * pj;
      aN[i][j] = (-kdiff[i][j] - 0.2*Tydy) * hxhy_sq * pj;   aS[i][j] = (-kdiff[i][j] + 0.2*Tydy) * hxhy_sq * pj;
      b[i][j] =  b[i][j] * pj  - 2.0 * rj * dx[0] * (-kdiff[i][j] - 0.2 * Txdx);
   }
  ...
  for(i=1; i<nx-1; i++)
   {
     pj = bcbot[i][0]; qj = bcbot[i][1]; rj = bcbot[i][2];
      ...
      aP[i][j] = 2.0 * (1.0 + hxhy_sq) * kdiff[i][j] * pj + 2.0 * dy[0] * qj * (-kdiff[i][j] + 0.2*Tydy) * hxhy_sq;
      aE[i][j] = -kdiff[i][j] - 0.2*Txdx;       aW[i][j] = -kdiff[i][j] + 0.2*Txdx;
      aN[i][j] = -2.0 * kdiff[i][j] * hxhy_sq * pj;   aS[i][j] = 0.0;
      b[i][j] =  b[i][j] * pj  + 2.0 * rj * dy[0] * (-kdiff[i][j] + 0.2*Tydy) * hxhy_sq;
  ...
  for(i=1; i<nx-1; i++)
   {
     ...
     Txdx = T[i+1][j] - T[i-1][j];
      ...
      aP[i][j] = 2.0 * (1.0 + hxhy_sq) * kdiff[i][j] * pj - 2.0 * dy[0] * qj * (-kdiff[i][j] - 0.2*Tydy) * hxhy_sq;
      aE[i][j] = -kdiff[i][j] - 0.2*Txdx;       aW[i][j] = -kdiff[i][j] + 0.2*Txdx;
      aN[i][j] = 0.0;   aS[i][j] = -2.0 * kdiff[i][j] * hxhy_sq * pj;
      b[i][j] =  b[i][j] * pj  - 2.0 * rj * dy[0] * (-kdiff[i][j] - 0.2*Tydy) * hxhy_sq;
   }
